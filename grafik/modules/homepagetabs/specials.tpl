...
{if isset($special) AND $special}
  		 {foreach from=$special item=product name=myLoop}
			...
			<div class="hpt_pricediv"> 	
				...
				{if isset($product.reduction_percent) && $product.reduction_percent}<span class="reduction">(-{$product.reduction_percent}%)</span>{/if}<br>
 		...
 		{/foreach}		 
	{else}
        <p>{l s='No Special Prodcuts' mod='homepagetabs'}   </p>
	{/if}	
 </div> <div style="clear:both;"></div>
<!-- /MODULE Home Specials -->