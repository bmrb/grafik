<!-- New Products -->
 ...
 {if isset($new_products) AND $new_products}
  		{foreach from=$new_products item=product name=myLoop} 
			...
			<span class="price_container"><span class="price"> {if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if} </span></span></div>		
 		...
 		{/foreach}		 
	{else}
        <p>{l s='No New Products' mod='homepagetabs'}</p>
	{/if}	
