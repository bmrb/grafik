<!-- Featured Products -->
 ...
 {if isset($featuredproducts)}
  		{foreach from=$featuredproducts item=product name=myLoop} 
			<div class="ajax_block_product hpt_eachproduct" {if $smarty.foreach.myLoop.index != 0 && $smarty.foreach.myLoop.index % 4 == 0}Style="clear:both"{/if}>
			 	...
			 	<div class="hpt_descproduct">
					<b><a href="{$product.link}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|replace:'  ':' '|replace:'\n':''|replace:'\r':''|truncate:$titlechars:'...'|escape:'htmlall':'UTF-8'}</a></b>
					<br />
					{if $desctype == 1}
	 			...
	 			{if $buttonstylecart == 1 || $buttonstyleview == 1}
					<div class="hpt_buttonpair">	
						...
						{if $buttonstylecart == 1} 
							<a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$product.id_product}" href="{$base_dir}cart.php?qty=1&amp;id_product={$product.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='homefeatured'}">{l s='Add to cart' mod='homepagetabs'}</a>
						{/if}
					</div>
				{/if}		
			</div>
 		{/foreach}		 
	{else}
        <p>{l s='No Featured Products' mod='homepagetabs'}</p>
	{/if}	
