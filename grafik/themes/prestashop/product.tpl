...
<div id="primary_block" class="clearfix">
    ...
    <b><span style="font-family:Arial, Helvetica, sans-serif; font-weight:bold; font-size:13px; color:#989898">&nbsp;&nbsp;{$product->name|escape:'htmlall':'UTF-8'}</span></b>
		...
		<div id="short_description_block" style="font-family:Arial, Helvetica, sans-serif; font-weight:normal; font-size:12px; color:#989898;">
			{if $product->description_short}
				<div id="short_description_content" class="rte align_justify" style="font-family:Arial, Helvetica, sans-serif; font-weight:normal; font-size:12px; color:#989898;">{$product->description_short}</div>
			{/if}

            <div>
        <b><span style="font-family:Arial, Helvetica, sans-serif; font-weight:bold; font-size:13px; color:#989898;">Image ID:</b></span><span style="font-size:16px">{$product->reference|escape:'htmlall':'UTF-8'}</span><br /><br />
       <b><span style="font-family:Arial, Helvetica, sans-serif; font-weight:bold; font-size:13px; color:#989898;">Requires Credits:</span></b> <span style="font-size:16px">{$tokens}</span><br /><br />
...
{if $tags|@count > 0}
     <div id="productpage_tag" style="font-family:Arial, Helvetica, sans-serif; font-weight:bold; font-size:13px; color:#989898;"> Keywords:<br />
        ...
        <a href="{$link->getPageLink('search.php')}?tag={$tag|urlencode}" style="font-family:Arial, Helvetica, sans-serif; font-weight:normal; font-size:12px; color:#989898; text-decoration:none">{$tag|escape:htmlall:'UTF-8'}</a>,&nbsp;
        {/foreach}
        </div>
{/if}
...
<!--			{if $product->online_only}
			...
			<p{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE} style="display: none;"{/if} id="add_to_cart" class="buttons_bottom_block"><input type="submit" name="Submit" value="{l s='Download'}" class="exclusive" /></p>
...
<div id="more_info_block" class="clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort">
		{if $product->description}<li><a id="more_info_tab_more_info" href="#idTab1"></a></li>{/if}
		{if $features}<li><a id="more_info_tab_data_sheet" href="#idTab2"></a></li>{/if}
		{if $attachments}<li><a id="more_info_tab_attachments" href="#idTab9"></a></li>{/if}
		{if isset($accessories) AND $accessories}<li><a href="#idTab4"></a></li>{/if}
		{$HOOK_PRODUCT_TAB}
	</ul>
...
{if $smarty.foreach.categoryProduct.index == 7}
    {break}
{/if} 
			<li class="ajax_block_product" style="width:110px">
				<a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'home')}" height="110px" width="110px" alt="{$categoryProduct.name|htmlspecialchars}" /></a><br/>
						...
						<li class="ajax_block_product {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
							...
							<p class="product_accessories_price">
								...
								{if ($accessory.allow_oosp || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
									<a class="exclusive button ajax_add_to_cart_button" href="{$link->getPageLink('cart.php')}?qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Download'}">{l s='Download'}</a>
								{else}
									<span class="exclusive">{l s='Download'}</span>
