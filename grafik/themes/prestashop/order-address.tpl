...
{/if}
	...
	outline: none" name="id_address_delivery" id="id_address_delivery" onchange="updateAddressesDisplay();{if $opc}updateAddressSelection();{/if}">
	...
	font-size: 12px;
	font-family: Verdana, Arial, Helvetica, Sans-Serif;
	outline: none" value="{$address.id_address|intval}" {if $address.id_address == $cart->id_address_delivery}selected="selected"{/if}>{$address.alias|escape:'htmlall':'UTF-8'}</option>
			{/foreach}
