		...
		<fieldset>
			...
			{if isset($customerThread.id_contact)}
				{foreach from=$contacts item=contact}
					{if $contact.id_contact == $customerThread.id_contact}
						<input type="text" class="text" id="contact_name" name="contact_name" value="{$contact.name|escape:'htmlall':'UTF-8'}" readonly="readonly" />
			...
			<p class="text">
				...
				{if isset($customerThread.email)}
					<input type="text" class="text" id="email" name="from" value="{$customerThread.email}" readonly="readonly" />
				{else}
					<input type="text" class="text" id="email" name="from" value="{$email}" />
				{/if}
			</p>
		{if !$PS_CATALOG_MODE}
			...
			<p class="text">
				...
				{elseif !isset($customerThread.id_order) && !isset($isLogged)}
					<input type="text" class="text" name="id_order" id="id_order" value="{if isset($customerThread.id_order) && $customerThread.id_order > 0}{$customerThread.id_order|intval}{else}{if isset($smarty.post.id_order)}{$smarty.post.id_order|intval}{/if}{/if}" />
				{elseif $customerThread.id_order > 0}
					<input type="text" class="text" name="id_order" id="id_order" value="{$customerThread.id_order|intval}" readonly="readonly" />
				{/if}
			</p>
			{/if}
